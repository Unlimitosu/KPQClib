/*
 * Test code for Peregrine implementation.
 * by Eun-Young Seo
 * Version: Nov. 16, 2022
 */
...
static void
test_sign_peregrine_self(const int8_t *f, const int8_t *g,
...
static void
test_keygen_peregrine_inner(unsigned logn, uint8_t *tmp)
{
	size_t n;
	int8_t *f, *g, *F, *G;
	uint16_t *h, *hm, *h2;
...
static void
sha1_print_line_with_int(sha1_context *sc, const char *s, unsigned x, FILE * f)
{
	char tmp[30];
	size_t u;
...
static void
test_nist_KAT(unsigned logn, const char *srefhash, FILE *fp_req, FILE *fp_rsp)
{
	int i;
	...
	//! hhc is real uninitialized
	unpoison(&hhc, sizeof(hhc));
	is_poisoned(&hhc, sizeof(hhc));

	sha1_print_line_with_int(&hhc, "# Peregrine-", (unsigned)n, fp_req);
    sha1_print_line_with_int(&hhc, "# Peregrine-", (unsigned)n, fp_rsp);
	sha1_print_line(&hhc, "");
	...
	for (i = 0; i < 1; i ++) {
		...
		sha1_print_line_with_hex(&hhc, "seed = ", seed, 48, fp_req);
        sha1_print_line_with_hex(&hhc, "seed = ", seed, 48, fp_rsp);
		sha1_print_line_with_int(&hhc, "mlen = ", (unsigned)mlen, fp_req);
        sha1_print_line_with_int(&hhc, "mlen = ", (unsigned)mlen, fp_rsp);
		sha1_print_line_with_hex(&hhc, "msg = ", msg, mlen, fp_req);
        sha1_print_line_with_hex(&hhc, "msg = ", msg, mlen, fp_rsp);
		sha1_print_line_with_hex(&hhc, "pk = ", pk, pk_len, fp_rsp);
		sha1_print_line_with_hex(&hhc, "sk = ", sk, sk_len, fp_rsp);
		sha1_print_line_with_int(&hhc, "smlen = ", (unsigned)smlen, fp_rsp);
		sha1_print_line_with_hex(&hhc, "sm = ", sm, smlen, fp_rsp);
        fprintf(fp_req, "pk = \n");
        fprintf(fp_req, "sk = \n");
