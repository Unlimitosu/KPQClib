...
static void check_eq(const void *a, const void *b, size_t len, const char *banner)
{
	size_t u;
 ...
 */
static void test_SHAKE256_KAT(const char *hexsrc, const char *hexout, uint8_t *tmp, size_t tlen)
{
	uint8_t *in, *out, *ref;
	size_t ilen, olen;
...
/*******************************/
/* Peregrine Verification Test */
/*******************************/

static void
test_vrfy_peregrine_inner(unsigned logn, const int8_t *f, const int8_t *g,
...
test_vrfy_peregrine(void)
{
	...
	test_vrfy_peregrine_inner(9, ntru_f_512, ntru_g_512, ntru_F_512, ntru_G_512,
		ntru_h_512, ntru_pkey_512, KAT_SIG_512, tmp, tlen);
	test_vrfy_peregrine_inner(10, ntru_f_1024, ntru_g_1024, ntru_F_1024, ntru_G_1024,
...
static void test_sign_peregrine_self(const int8_t *f, const int8_t *g, const int8_t *F, const int8_t *G, const uint16_t *h_src, unsigned logn, uint8_t *tmp)
{
    int iteration = 3;
    size_t n;
    char buf[20];
    ...
    for (int i = 0; i < iteration; i ++) {//number of test=iteration
        printf("i:%d\n",i);
        uint8_t msg[50];  /* nonce + plain */
        inner_shake256_context sc, sc2;
        size_t u;
        
        //Message source generation and hashing
        ...
        //Signature for a given hm
        Zf(sign_dyn_peregrine)(sig, &sc, f, g, F, G, hm, h_src, logn, tt);
...
static void test_sign_peregrine(void)
{
    uint8_t *tmp;
    size_t tlen;
    ...
    test_sign_peregrine_self(ntru_f_512, ntru_g_512, ntru_F_512, ntru_G_512, ntru_h_512, 9, tmp);
    //test_sign_peregrine_self(ntru_f_1024, ntru_g_1024, ntru_F_1024, ntru_G_1024, ntru_h_1024, 10, tmp);
...
static void test_keygen_peregrine_inner(unsigned logn, uint8_t *tmp)
{
	size_t n;
	int8_t *f, *g, *F, *G;
	uint16_t *h, *hm, *h2;
...
/*********************************/
/* Peregrine Key Generation Test */
/*********************************/

static void test_keygen_peregrine(void)
{
	uint8_t *tmp;
	size_t tlen;
	unsigned logn;
	...
	for (int i = 0; i < iteration; i++) {
		test_keygen_peregrine_inner(9, tmp);
	}
	xfree(tmp);
	printf("done.\n");
	fflush(stdout);
...
static inline uint32_t dec32be(const void *src)
{
...
static inline void enc32be(void *dst, uint32_t x)
{
	unsigned char *buf;
...
static void aes256_keysched(uint32_t *skey, const void *key)
{
	int i, j, k;
...
static void aes256_encrypt(const uint32_t *skey, void *data)
{
	unsigned char *buf;
...
static void sha1_round_inner(const uint8_t *buf, uint32_t *val)
{
...
static void sha1_init(sha1_context *sc)
{
...
static void sha1_update(sha1_context *sc, const void *data, size_t len)
{
	const uint8_t *buf;
	size_t ptr;
...
static void sha1_out(const sha1_context *cc, void *dst)
{
	uint8_t buf[64];
	uint32_t val[5];
	size_t ptr;
...
static void sha1_print_line(sha1_context *sc, const char *s)
{
	char c;
...
static void sha1_print_line_with_int(sha1_context *sc, const char *s, unsigned x, FILE * f)
{
	char tmp[30];
	size_t u;
...
static void sha1_print_line_with_hex(sha1_context *sc,
	const char *s, const void *data, size_t len, FILE *f)
{
	const uint8_t *buf;
	char c;
...
static void aes256_DRBG_update(const uint8_t *provided_data, uint8_t *key, uint8_t *V)
{
	uint32_t skey[60];
	uint8_t tmp[48];
	int i, j;
...
static void nist_randombytes_init(uint8_t *entropy_input)
{
...
static void nist_randombytes(uint8_t *buf, size_t len)
{
	while (len > 0) {
		unsigned cc;
		uint32_t skey[60];
...
//start
    ...
    size_t sk_len, pk_len, over_len;
    sk_len = (logn == 9) ? 1281 : 2305;
	pk_len = (logn == 9) ? 897 : 1793;
	over_len = (logn == 9) ? 690 : 1335;
	...
	for (i = 0; i < 1; i ++) {
		...
		//u = Zf(comp_encode)(sm + 43 + mlen, over_len - 43, sig, logn); ///original falcon
        u = Zf(comp_encode)(sm + 43 + mlen, over_len - 17, sig, logn);//-17 is okay but -16 is not okay for n=1024
		if (u == 0) {
