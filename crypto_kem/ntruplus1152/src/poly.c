...
void poly_cbd1(poly *a, const unsigned char buf[NTRUPLUS_N/4])
{
	...
	for(int j = 0; j < 4; j++)
	{
		t1 = load32_littleendian(buf + 4*j + 128 );
		t2 = load32_littleendian(buf + 4*j + 128 + NTRUPLUS_N/8);

		for (int k = 0; k < 2; k++)
		{
			for(int l = 0; l < 16; l++)
			{
				a->coeffs[1024 + 8*l + 2*j + k] = (t1 & 0x1) - (t2 & 0x1);
...
void poly_sotp(poly *e, const unsigned char *msg, const unsigned char *buf)
{
	...
	for(int j = 0; j < 4; j++)
	{
		t1 = load32_littleendian(tmp + 4*j + 128);
		t2 = load32_littleendian(tmp + 4*j + 128 + NTRUPLUS_N/8);

		for (int k = 0; k < 2; k++)
		{
			for(int l = 0; l < 16; l++)
			{
				e->coeffs[1024 + 8*l + 2*j + k] = (t1 & 0x1) - (t2 & 0x1);
...
void poly_sotp_inv(unsigned char *msg, const poly *e, const unsigned char *buf)
{
	...
	for(int j = 0; j < 4; j++)
	{
		...
		for (int k = 0; k < 2; k++)
		{
			for(int l = 0; l < 16; l++)
			{
				t3 ^= (((e->coeffs[1024 + 8*l + 2*j + k] + t2)^t1) & 0x1) << (l+16*k);
